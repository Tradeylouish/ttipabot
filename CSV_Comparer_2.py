...
def excel_diff(path_OLD, path_NEW, index_col):
    ...
    df_OLD = pd.read_csv(path_OLD, index_col=index_col).fillna(0)
    df_NEW = pd.read_csv(path_NEW, index_col=index_col).fillna(0)
...
def main():
    ...
    path_OLD = Path(date1_filename)
    path_NEW = Path(date2_filename)
